...
void Graphics::addWireCube()
{
    std::vector<float> cubeData;
    cubeData.resize(cubeVertexCount * 8);
    std::copy(&cubeVertexBufferData[0], &cubeVertexBufferData[cubeVertexCount * 8 - 1], cubeData.begin());
    addShape("wire_cube", cubeData, VBO::GEOMETRY_LAYOUT::LAYOUT_LINES);
}

void Graphics::addWireCylinder()
{
    std::vector<float> cylinderData;
    cylinderData.resize(cylinderVertexCount * 8);
    std::copy(&cylinderVertexBufferData[0], &cylinderVertexBufferData[cylinderVertexCount * 8 - 1], cylinderData.begin());
    addShape("wire_cylinder", cylinderData, VBO::GEOMETRY_LAYOUT::LAYOUT_LINES);

    // Update shape
    std::shared_ptr<Shape> shape = getShape("wire_cylinder");
    shape->translate(glm::vec3(0, 0.5f, 0));
}

void Graphics::addWireSphere()
{
    std::vector<float> sphereData;
    sphereData.resize(sphereVertexCount * 8);
    std::copy(&sphereVertexBufferData[0], &sphereVertexBufferData[sphereVertexCount * 8 - 1], sphereData.begin());
    addShape("wire_sphere", sphereData, VBO::GEOMETRY_LAYOUT::LAYOUT_LINES);
}
...
void Graphics::drawShape(std::shared_ptr<Shape> shape) {
    ...
    if(!shape->m_built) {
        if(!shape->printDebug()) {
            std::cerr << "The above warnings pertain to shape '" << shape->getShapeName() << "'" << std::endl;
            std::cerr << std::endl;
        }
    }
#endif
...
void Graphics::setCamera(const std::shared_ptr<CCamera> &cam) {
    std::shared_ptr<Camera> camera = cam->getCameraObject();
...
std::shared_ptr<CCamera> Graphics::getActiveCamera() {
...
void Graphics::drawFramebuffer(const std::shared_ptr<FBO> &fbo, const glm::vec2 &pos, const glm::vec2 &size) {
    if(m_activeCamera != nullptr && m_activeCamera->getCameraObject()->isUI()) {
        // Transform
