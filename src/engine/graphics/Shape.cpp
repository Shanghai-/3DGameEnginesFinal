...
Shape::Shape(const std::vector<float> &positions,
    ...
    m_scale(1),
    m_rotation(),
    m_bounds(glm::vec3(0.f), glm::vec3(0.f))
{
    addPositions(positions);
    addNormals(normals);
    addTextureCoordinates(texCoords);
...
void Shape::translate(glm::vec3 delta) {
    m_pos += delta;
    m_bounds.updateBounds(glm::translate(delta));
}
...
bool Shape::printDebug() {
    ...
    //std::cerr << "Pos vec size: " << m_positions.size() << std::endl;
